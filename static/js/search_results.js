...
function downloadTableAsTSV(filename) {
  let tsvContent = "Source\tInteraction Type\tTarget\tPubmed ID\n";
  var decoded = decodeHTMLEntities(g).replace(
    /'(\b(?:id|target|inter_type|publication)\b)':/g,
    '"$1":'
  );
...
function showPaperPopup(pubmedID, source, typa, target) {
  ...
  $.get(pubmedURL, function (data) {
    ...
    for (var i = 0; i < sourceWords.length; i++) {
      var sourceRegex = new RegExp(
        "\\b" +
          sourceWords[i].replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&") +
          "\\b",
        "gi"
      );
      underlinedData = underlinedData.replace(
        sourceRegex,
        "<u><span class='red-text'>$&</span></u>"
      );
    }
    for (var j = 0; j < targetWords.length; j++) {
      var targetRegex = new RegExp(
        "\\b" +
          targetWords[j].replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&") +
          "\\b",
        "gi"
      );
      underlinedData = underlinedData.replace(
    ...
    var popupHTML =
      ...
      "</h2><pre>" +
      underlinedData +
      "</pre> Click <a href='https://pubmed.ncbi.nlm.nih.gov/" +
      pubmedID +
      "'target='_blank' class='pubmed-hyperlink'>here</a> to view the publication.</div>";
